...
%N = 200; % Número de amostras para f=0.05==>N=200
%N=167;
...
%%%%%%%%%%%%%%%%%%%%testando
FuzzyAFILC = readfis('FuzzyRogerrgpc6');
%%%%%%%%%%%%%%%%%%%%%%%
...
%ve_li(18000:27000) = 25;%0.05hz, meno frequência do bulging
...
switch Tipo_intervalo
    case 1        
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%teste de 200 para 190 - cenário 1  %%%%%%%%%%%%%%%%%%%%
if(N==200) f=0.05; f1=0.0526; vl1=10; vl2=10.52; end; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
case 2
%%%%%%%%%%%%%%%%%%%%%%%%teste limite RGPC-200 para 185-cenário 2 %%%%%%%%%%%%%%%%%%%%%%%%2%%%%%%%%%%%%
if(N==200) f=0.05; f1=0.054; vl1=10; vl2=10.80; end; 
%%%%%%%%%%%
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
 case 3
%%%%%%%%%%%%%%%%%%%%%%%%teste limite RGPC-200 para 177- cenário 3
if(N==200) f=0.05; f1=0.0565; vl1=10; vl2=11.20; end; 
%%%%%%%%%%%%%%%%%%%%%%%%%% 
case 4
  %%%%%%%%%%%%%%%%%%%%%%%%teste limite RGPC-200 para 175
% if(N==200) f=0.05; f1=0.057; vl1=10; vl2=11.4; end; %f=0.05 N=200 vl=1
% end

% if(N==100) f=0.1; f1=0.113; vl1=10; vl2=11.4; end; %f=0.05 N=200 vl=1
% end
...
% plot(Blg)
dist=Blg; 
%vl = 20;  %valor inicial da velocidade de lingotamento
Xs = [35 100 100];       % ponto de operaçao
%Xs =[ posicao da válvula     nível do molde      medição do sensor]
 ...
 % Sintonia do Controlador PID
  ...
  G = zeros(hp,hc); %inicializando a matriz G
  y=zeros(II+hp,1); %inicializando a saida da planta com preditivo
  y_n=zeros(II+hp,1);
  y_r=zeros(II+hp,1);
  Du=zeros(II+hp,1); %inicializa o vetor do diferencial de controle
  Du_n=zeros(II+hp,1);
  u=zeros(II+hp,1);%inicializa o vetor de controle do sistema
  u_n=zeros(II+hp,1);
  u_r=zeros(II+hp,1);
  DNu=zeros(II+hp,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
%%%%Polinomios A e B
  ...
  while (ts < tfinal)
    ...
    %Realizando a Equação da Diofantina
    [E_r,F_r] = diofantina(Atil_r,hp);

    [E_n,F_n] = diofantina(Atil_n,hp);
    ...
    %Pegando os parâmetros da Matriz H
    gy_r = H_r(hp,:)';
    gy_n = H_n(hp,:)';
    ...
    iii=0;
    %G_linha = zeros(hp,hp);
    for iii = 1:hp %Numero de linhas igual ao horizonte de predicao
        G_aux2_r = H_r(d+iii,:);
        G_aux2_n = H_n(d+iii,:);
        G2_r = G1_r(iii,:);
        G2_n = G1_n(iii,:);
    ...
    %%Passo 2 = Calcular ação de controle Du_n
    % Calculando a Referencia Futura
    %i=0;
    ...
    a = u_n(k_geral,1);
    %%Passo 3 = Obter yr usando o valor medido da planta
...
%===========================  PID   ==============================
 ...
 IAE_Total = IAE_Total + abs(erro(k_geral));
    ...
    yf_r = yf1_r + yf2_r;
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    %Restriçao na Açao de Controle u
 umax = 70;
 %umax = 30;
 umin = 0;
 I = eye(hc,hc);
...
%Restrição na Variação do Sinal de Controle
 Dumax = 80;
 Dumin = -80;
 S0 = eye(hc,hc);
 [l,c] = size(S0);
...
%Restriçao na Variação N-peridica da ação de controle
  DNumax = 80;
  DNumin = -80;
  I = eye(hc,hc);
  II = [I;-I];
    ...
    break
     ...
     Du(k_geral,1) = u(k_geral,1) - u(k_geral-1,1);
        ...
        if(ts>100)% aguarda acabar o estado transitoria
         if (k<150)% o IAE será calculado só nas primeiras 150 amostras
            IAEj = IAEj + abs(ej(k));
         end
         ...
         if (k==N)  %===========
             ...
             if j_ilc > 1   % Os dois primeiros foram inicializados como zero
                 ...
                 %%%%%%%%%%%%%%%%%%normal
                 N = N+HabFuzzy*delta; % Número de amostras para f=0.05==>N=200
...
set(handles.text2,'Visible','off');
      ...
      %     %%%%%%%%%%%%%%%%%%%%%%%%%retirada IAE , ZELEANDRO 
  subplot(2,1,1,'Parent',handles.uipanel4); plot(vet_tempo, vet_nivel,'k', vet_tempo, vet_u,'b'); 
  ylabel('Nível(mm)'); xlabel('Tempo(s)'); grid on; xlim([116 2000]); ylim([95 105]);
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%sem N(samples)
  ylabel('N(amostras)'); xlabel('Tempo(s)'); grid on; xlim([116 2000]); ylim([170 210]);
 % legend('N'); xlabel('Time(s)'); grid on; xlim([116 2000]); ylim([195 205]);
 disp('Processamento Concluído. Veja as imagens na interface.');
